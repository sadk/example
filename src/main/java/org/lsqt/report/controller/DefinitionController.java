...
public class DefinitionController {
	...
	public void export(DefinitionQuery query, String exportFileType, String exportDataType) {
		Page<Definition> page = db.getEmptyPage();
		...
		if (Dictionary.EXPORT_FILE_TYPE_EXCEL.equals(exportFileType)) {
			...
			if (Dictionary.EXPORT_DATA_TYPE_全部数据.equals(exportDataType)) {
				...
				page.setData(definitionService.getAll());
				ContextUtil.file.put("/template/应用列表数据.xls", "page", page);
				return ;
			} 
			
			page = definitionService.queryForPage(query);
			ContextUtil.file.put("/template/应用列表数据.xls", "page", page);
