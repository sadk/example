...
ALTER TABLE `sys_tenant` 
CHANGE COLUMN `full_name` `full_name` VARCHAR(50) NULL DEFAULT NULL COMMENT '租户全称' AFTER `code`,
CHANGE COLUMN `nick_name` `nick_name` VARCHAR(50) NULL DEFAULT NULL COMMENT '租户昵称' AFTER `full_name`,
CHANGE COLUMN `introduction` `introduction` VARCHAR(1024) NULL DEFAULT NULL COMMENT '租户文字介绍' AFTER `nick_name`,
CHANGE COLUMN `value` `value` VARCHAR(50) NULL DEFAULT NULL COMMENT '租户值' ;



ALTER TABLE `sys_file`  DROP COLUMN `obj_id`;
ALTER TABLE `sys_file`  CHANGE COLUMN `data_type` data_type INT(4) NULL COMMENT '业务数据类型（从字典引用值）,1=企业logo 2=职位封面 3=职位视频' ;
...
ALTER TABLE `rpt_definition` CHANGE COLUMN `data_replica_stroe_precision` `data_replica_storage_precision` INT(2) NULL DEFAULT NULL COMMENT '数据副本存储精度模式 1=全字段按字符存储  2=按导入定义字段类型存储' ;
-- ALTER TABLE `rpt_definition` ADD COLUMN `report_script` TEXT NULL  comment '报表自定义脚本,用于自定义js函数，自行控制调用' ;
-- ALTER TABLE `rpt_definition`  ADD COLUMN `count_required` INT(2) NULL COMMENT '分页是否统计总记录数' ;
...
ALTER TABLE  `sys_machine`   CHANGE COLUMN `user_name` `user_name` VARCHAR(256) NULL COMMENT '登陆名称' , CHANGE COLUMN `user_password` `user_password` VARCHAR(256) NULL COMMENT '登陆密码' ;
...
-- ALTER TABLE  `uum_res`  ADD COLUMN `url` VARCHAR(2000) NULL ;
 ALTER TABLE  `uum_res` CHANGE COLUMN `icon` icon VARCHAR(200) NULL  ;
...
ALTER TABLE `bu_job_info` CHANGE COLUMN `job_id` `job_id` VARCHAR(50) NOT NULL COMMENT '职位ID' ;
...
ALTER TABLE `bu_job_address_relationship` CHANGE COLUMN `job_id` `job_id` VARCHAR(50) NULL DEFAULT '' , CHANGE COLUMN `addr_id` `addr_id` VARCHAR(50) NULL DEFAULT NULL ;
...
ALTER TABLE `bu_job_welfare_relationship` CHANGE COLUMN `welfare_id` `welfare_id` VARCHAR(50) NULL DEFAULT NULL COMMENT '福利ID' ;
ALTER TABLE `bu_job_welfare_relationship` CHANGE COLUMN `job_id` `job_id` VARCHAR(50) NULL DEFAULT NULL COMMENT '职位ID' ;
...
ALTER TABLE  `bu_job_info` CHANGE COLUMN `company_id` `company_id` VARCHAR(50) NOT NULL COMMENT '公司ID' , CHANGE COLUMN `announcer_id` `announcer_id` VARCHAR(50) NULL DEFAULT NULL COMMENT '发布者编码' ;



ALTER TABLE  `uum_user`  CHANGE COLUMN `code` `code` VARCHAR(50) NULL DEFAULT NULL COMMENT '用户编码' , ADD COLUMN `tenant_code` VARCHAR(50) NULL COMMENT '用户所属租户码' ;
ALTER TABLE  `uum_user`  ADD COLUMN `tenant_name` VARCHAR(50) NULL AFTER `tenant_code`;
...
ALTER TABLE  `bu_code_library`  CHANGE COLUMN `code_no` `code_no` VARCHAR(50) NOT NULL COMMENT '编码' , CHANGE COLUMN `item_no` `item_no` VARCHAR(200) NULL DEFAULT NULL COMMENT '项目编号' ;


ALTER TABLE `bu_job_video`  CHANGE COLUMN `video_id` `video_id` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '视频id' , CHANGE COLUMN `job_id` `job_id` VARCHAR(50) NULL DEFAULT NULL COMMENT '职位ID' ;


ALTER TABLE `bu_job_info`  CHANGE COLUMN `picture_url` `picture_url` VARCHAR(1000) NULL DEFAULT NULL COMMENT '企业logo图' , CHANGE COLUMN `cover_url` `cover_url` VARCHAR(1000) NULL DEFAULT NULL COMMENT '职位的封面图' ;


 -- --------------------------------------------------- 字段加长
ALTER TABLE `bu_job_info_backup`  CHANGE COLUMN `job_id` `job_id` VARCHAR(50) NULL DEFAULT NULL COMMENT '职位编码' ;

ALTER TABLE `bu_job_info_extension`  CHANGE COLUMN `job_id` `job_id` VARCHAR(50) NOT NULL ;
...
ALTER TABLE `bu_job_relationship`  CHANGE COLUMN `job_id` `job_id` VARCHAR(50) NULL DEFAULT NULL COMMENT '职位编码' ;
...
ALTER TABLE `bu_job_user_statistics`  CHANGE COLUMN `job_id` `job_id` VARCHAR(50) NULL DEFAULT NULL COMMENT '职位ID' ;
...
ALTER TABLE `bu_store_with_manager` CHANGE COLUMN `manager_id` `manager_id` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '管理者id' ;
ALTER TABLE `bu_account_info` CHANGE COLUMN `account_id` `account_id` VARCHAR(50) NULL DEFAULT NULL COMMENT '账户编码' ;
ALTER TABLE `bu_activity_info`  CHANGE COLUMN `company_id` `company_id` VARCHAR(50) NULL DEFAULT NULL COMMENT '公司ID' ;
...
ALTER TABLE `bu_job_info` CHANGE COLUMN `work_addr_id` `work_addr_id` VARCHAR(50) NULL DEFAULT NULL COMMENT '工作地址ID' ;


ALTER TABLE `bu_personal_video_info`  CHANGE COLUMN `job_id` `job_id` VARCHAR(50) NULL DEFAULT NULL COMMENT '职位ID' ;


ALTER TABLE `bu_red_packet`  CHANGE COLUMN `job_id` `job_id` VARCHAR(50) NOT NULL ;

ALTER TABLE `bu_store_info`  CHANGE COLUMN `store_id` `store_id` VARCHAR(50) NOT NULL DEFAULT '门店id' ;


ALTER TABLE `bu_store_user_relationship`  CHANGE COLUMN `store_id` `store_id` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '门店id' ;


ALTER TABLE `bu_store_with_manager` CHANGE COLUMN `store_id` `store_id` VARCHAR(50) NOT NULL DEFAULT '' COMMENT '门店id' ;
...
UPDATE bu_job_info A,  bu_job_info_extension B  
SET 
    A.responsibility = B.other_demands,
