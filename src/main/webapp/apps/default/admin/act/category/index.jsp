...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<div class="mini-splitter" style="width:100%;height:100%;">
			...
			<div showCollapseButton="true">
				...
				<div class="mini-fit">
					...
					url="${pageContext.request.contextPath}/act/category/all" > 
		...
		function repairNodePath() {
			$.ajax({
				'url': "${pageContext.request.contextPath}/act/category/repair_node_path",
				type: 'post',
				dataType:'JSON',
				cache: false,
				async:false,
			...
			if(row) {
				mini.confirm("删除当前分类，其子分类也即将删除，确定删除？", "确定？",
						function (action) {
							if (action == "ok") {
								$.ajax({
									'url': "${pageContext.request.contextPath}/act/category/delete?ids="+row.id,
									type: 'post',
									dataType:'JSON',
									cache: false,
									async:false,
		...
		function add() {
			var row = grid.getSelected();
			mini.open({
				url : "${pageContext.request.contextPath}/apps/default/admin/act/category/edit.jsp",
				title : "添加分类",
				width : 500,
				height : 300,
				onload : function() {
