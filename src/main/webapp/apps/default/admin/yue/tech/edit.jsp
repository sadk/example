	...
	<body> 
		            ...
		            <div style="padding:5px;">
				        <table>
							...
							<tr>
								<td>数据类型：</td>
								<td>
								 	 <input id="type" name="type" class="mini-combobox" showNullItem="true" nullItemText="请选择..." emptyText="请选择" textField="name" valueField="value" url="${pageContext.request.contextPath}/dictionary/option?code=tech_definition_data_type" />   
								</td> 
								<td>上级：</td>
								<td>
		...
		<script type="text/javascript">
	        ...
	        function onButtonEdit(e) {
	            ...
	            //alert(idNotIn);
	            mini.open({
	                url: "${pageContext.request.contextPath}/apps/default/admin/yue/tech/selector_tech.jsp?idNotIn="+idNotIn,
	                title: "选择列表",
	                width: 650,
	                height: 380,
			...
			function SaveData() {
				...
				//if(true) return ;
				$.ajax({
					url : "${pageContext.request.contextPath}/yue/tech/save_or_update",
					dataType: 'json',
					type : 'post',
					cache : false,
					data: form.getData(),
			...
			function SetData(data) {
				data = mini.clone(data); //跨页面传递的数据对象，克隆后才可以安全使用
				 ...
				 if(data.action == "edit" || data.action=='view') {
					$.ajax({
						url : "${pageContext.request.contextPath}/yue/tech/page?id=" + data.id,
						dataType: 'json',
						cache : false,
						success : function(text) {
