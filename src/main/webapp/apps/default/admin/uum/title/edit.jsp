	...
	<body> 
		            ...
		            <div style="padding:5px;">
				        <table>
							...
							<tr>
								<td>称谓类型</td>
								<td>
									<input id="type" name="type" class="mini-combobox" showNullItem="true" nullItemText="请选择..." emptyText="请选择" textField="name" valueField="value" url="${pageContext.request.contextPath}/dictionary/option?code=title_type"  />
								</td>
								<td>序号：</td>
								<td>
		...
		<script type="text/javascript">
			...
			function SaveData() {
				...
				//if(true) return ;
				$.ajax({
					url : "${pageContext.request.contextPath}/title/save_or_update",
					dataType: 'json',
					type : 'post',
					cache : false,
					data: form.getData(),
			...
			function SetData(data) {
				data = mini.clone(data); //跨页面传递的数据对象，克隆后才可以安全使用
				 ...
				 if(data.action == "edit" || data.action=='view') {
					$.ajax({
						url : "${pageContext.request.contextPath}/title/page?id=" + data.id,
						dataType: 'json',
						cache : false,
						success : function(text) {
