	...
	<body> 
		            ...
		            <div style="padding:5px;">
				        <table>
							...
							<tr>
								...
								<td>上级组：</td>
								<td>
								 	<input id="pid" name="pid" class="mini-buttonedit" onbuttonclick="onButtonEdit" emptyText="上级组可以为空"/>    
								</td>
							</tr>
		...
		<script type="text/javascript">
	        ...
	        function onButtonEdit(e) {
	            ...
	            //alert(idNotIn);
	            mini.open({
	                url: "${pageContext.request.contextPath}/apps/default/admin/uum/group/selector_group.jsp?idNotIn="+idNotIn,
	                title: "选择列表",
	                width: 650,
	                height: 380,
			...
			function SaveData() {
				...
				//if(true) return ;
				$.ajax({
					url : "${pageContext.request.contextPath}/group/save_or_update",
					dataType: 'json',
					type : 'post',
					cache : false,
					data: form.getData(),
			...
			function SetData(data) {
				data = mini.clone(data); //跨页面传递的数据对象，克隆后才可以安全使用
				 ...
				 if(data.action == "edit" || data.action=='view') {
					$.ajax({
						url : "${pageContext.request.contextPath}/group/page?id=" + data.id,
						dataType: 'json',
						cache : false,
						success : function(text) {
