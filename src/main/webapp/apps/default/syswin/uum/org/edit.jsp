...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body> 
		<form id="edit-form1" method="post" style="height:97%; overflow:auto;">
			...
			<input name="nodePath" class="mini-hidden" />
				        ...
				        <table>
							<tr>
								...
								<td style="width:150px;">
								 	<input name="fullName" id="fullName" class="mini-textbox"  emptyText="请输入名称"/>
								</td>
								<td style="width:100px;">简称：</td>
								<td style="width:150px;">
									<input name="name" id="name" class="mini-textbox" emptyText="请输入简称"/>
								</td>
							</tr>
							<tr>
								<td>组织单元级别：</td>
								<td> 
								 	<input id="type" name="type" class="mini-combobox" showNullItem="true" nullItemText="请选择..." emptyText="请选择" textField="name" valueField="value"
								 	 data="[{name:'集团',value:1},{name:'区域公司',value:2},{name:'公司',value:3},{name:'管理项目',value:4},{name:'部门',value:5},{name:'工作组',value:6}]" />
								</td>
		...
		<script type="text/javascript">
	        ...
	        function onButtonEdit(e) {
	            ...
	            //alert(idNotIn);
	            mini.open({
	                url: "${pageContext.request.contextPath}/apps/default/syswin/uum/org/selector_org.jsp?idNotIn="+idNotIn,
	                title: "选择列表",
	                width: 650,
	                height: 380,
			...
			function SaveData() {
				...
				//if(true) return ;
				$.ajax({
					url : "${pageContext.request.contextPath}/syswin/org/save_or_update",
					dataType: 'json',
					type : 'post',
					cache : false,
					data: form.getData(),
			...
			function SetData(data) {
				data = mini.clone(data); //跨页面传递的数据对象，克隆后才可以安全使用
				 ...
				 if(data.action == "edit" || data.action=='view') {
					$.ajax({
						url : "${pageContext.request.contextPath}/syswin/org/page?id=" + data.id,
						dataType: 'json',
						cache : false,
						success : function(text) {
