	...
	<body> 
		            ...
		            <div style="padding:5px;">
				        <table>
							...
							<tr>
								<td>上级菜单：</td>
								<td>
								 	<input id="pid" name="pid" class="mini-buttonedit" onbuttonclick="onButtonEdit" emptyText="上级菜单可以为空"/>    
								</td>
								<td>URL：</td>
								<td> 
								 	<input id="url" name="url" class="mini-textbox" emptyText="请输入备注"/>
								</td>
							</tr>
							...
							<tr>
								...
								<td>备注：</td>
								<td> 
								 	<input id="description" name="description" class="mini-textbox" emptyText="请输入备注"/>
								</td>
		...
		<script type="text/javascript">
	        ...
	        function onButtonEdit(e) {
	            ...
	            //alert(idNotIn);
	            mini.open({
	                url: "${pageContext.request.contextPath}/apps/default/syswin/uum/menu/selector_menu_single.jsp?idNotIn="+idNotIn,
	                title: "菜单列表",
	                width: 700,
	                height: 430,
			...
			function SaveData() {
				...
				//if(true) return ;
				$.ajax({
					url : "${pageContext.request.contextPath}/syswin/menu/save_or_update",
					dataType: 'json',
					type : 'post',
					cache : false,
					data: form.getData(),
			...
			function SetData(data) {
				data = mini.clone(data); //跨页面传递的数据对象，克隆后才可以安全使用
				 ...
				 if(data.action == "edit" || data.action=='view') {
					$.ajax({
						url : "${pageContext.request.contextPath}/syswin/menu/page?id=" + data.id,
						dataType: 'json',
						cache : false,
						success : function(text) {
